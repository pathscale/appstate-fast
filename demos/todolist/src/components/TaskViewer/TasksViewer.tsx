      ...
      >
        ...
        <div style={{ flexGrow: 2 }}>
          <input
            ...
            value={settingsState.isEditableInline ? taskNameGlobal.get() : taskNameLocal.get()}
            onChange={e => {
              if (settingsState.isEditableInline) {
                // taskNameGlobal.set(e.target.value);
                taskNameGlobal.set((e.target as HTMLInputElement)?.value)
              }
              // taskNameLocal.set(e.target.value);
              taskNameLocal.set((e.target as HTMLInputElement)?.value)
            }}
          />
        </div>
      </div>
...
export const TasksViewer = defineComponent(() => {
  ...
  if (loading) {
    return () => <div style={{ textAlign: 'center' }}>Loading initial state asynchronously...</div>
  }
