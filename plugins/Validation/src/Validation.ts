...
class ValidationPluginInstance<S> {
  ...
  getRulesAndNested(path: Path): [ValidationRule[], string[]] {
    ...
    return [
      result && result[PluginID] ? Array.from(result[PluginID].values()) : [],
  ...
  getErrors(
    ...
    if (Array.isArray(nestedInst)) {
      if (nestedRulesKeys.includes('*')) {
        for (let i = 0; i < nestedInst.length; i += 1) {
          ...
          result = result.concat(
            Validation(n as State<StateValueAtPath>).errors(filter, depth - 1, first),
          )
      ...
      for (let i = 0; i < nestedRulesKeys.length; i += 1) {
        ...
        if (nestedInst[k] !== undefined) {
          result = result.concat(
            Validation(nestedInst[k] as State<StateValueAtPath>).errors(filter, depth - 1, first),
          )
          if (first && result.length > 0) {
            return result
          }
  ...
  if ($this) {
    ...
    return {
      ...
      errors: (filter, depth, first) => {
        return inst.getErrors(
          state,
          depth === undefined ? Number.MAX_SAFE_INTEGER : depth,
          filter,
          first,
        )
      },
      firstError: (filter, depth) => {
        const r = inst.getErrors(
          state,
          depth === undefined ? Number.MAX_SAFE_INTEGER : depth,
          filter,
          true,
