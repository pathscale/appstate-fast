...
function DevToolsInitializeInternal() {
  ...
  function getLabel(isGlobal?: boolean) {
    ...
    return parts[2].replace(/\s*[(].*/, '').replace(/\s*at\s*/, '')
  }

  function createReduxDevToolsLogger(
    lnk: State<StateValueAtRoot>,
    assignedId: string,
    onBreakpoint: () => void,
  ) {
        ...
        return {
          ...
          label(name: string) {
            ...
            if (isMonitored(name, true)) {
              ...
              MonitoredStatesLogger(
                `RENAME '${assignedName}' => '${name}' (unmonitored => unmonitored)`,
              )
            }
            assignedName = name
          },
          onSet:
            submitToMonitor &&
            ((p: PluginCallbacksOnSetArgument) => {
              submitToMonitor!({ ...p, type: `SET [${p.path.join('/')}]` })
              if (breakpoint) {
                // tslint:disable-next-line: no-debugger
                debugger
