...
test('logger: should log objects untracked', async () => {
  ...
  try {
    ...
    const { result } = renderHook(() => {
      renderTimes += 1
      return useState({ prop: 1 }).attach(Logger)
    })
...
test('logger: should log undefined prop', async () => {
  ...
  try {
    ...
    const { result } = renderHook(() => {
      renderTimes += 1
      return useState<{ prop: undefined | null }>({ prop: undefined }).attach(Logger)
    })
    // tslint:disable-next-line: no-console
    expect(console.log).toHaveBeenCalledTimes(1)
...
test('logger: should log undefined value', async () => {
  ...
  try {
    ...
    const { result } = renderHook(() => {
      renderTimes += 1
      return useState<null | undefined>(undefined).attach(Logger)
    })
    // tslint:disable-next-line: no-console
    expect(console.log).toHaveBeenCalledTimes(1)
