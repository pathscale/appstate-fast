import { self } from '@appstate-fast/core';
...
var LoggerPluginInstance = /** @class */ (function () {
    ...
    LoggerPluginInstance.prototype.onSet = function (p) {
        ...
        console.log("[appstate-fast]: new value set at path '/" + p.path.join('/') + "': " +
            ("" + this.toJsonTrimmed(p.value)), p);
    };
    LoggerPluginInstance.prototype.log = function (path, l) {
        // tslint:disable-next-line: no-console
        return console.log("[appstate-fast]: current value at path '/" + path.join('/') + ": " +
...
function Logger($this) {
    ...
    return {
        ...
        init: function () {
            // tslint:disable-next-line: no-console
            console.log("[appstate-fast]: logger attached");
