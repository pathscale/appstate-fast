import { createState, useState, self } from '@appstate-fast/core';
...
const state = createState<Task[]>(new Promise(resolve => {
    ...
    setTimeout(() => resolve([
        {
            id: '1',
            name: 'Discover Appstate-fast',
            done: true,
        }, {
            id: '2',
            name: 'Replace Redux by Appstate-fast',
            done: false,
        }, {
            id: '3',
...
setTimeout(() => state[state.length][self].set({
    id: '100',
    name: 'Spread few words about Appstate-fast',
    done: false
}), 10000)