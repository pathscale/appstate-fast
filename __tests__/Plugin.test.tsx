...
test('plugin: common flow callbacks', async () => {
    ...
    return useState([
      ...
      init: () => {
        ...
        return {
          onBatchStart: p => {
            messages.push(
              `onBatchStart called, [${p.path}]: ${JSON.stringify(
                p.state,
          ...
          onBatchFinish: p => {
            messages.push(
              `onBatchFinish called, [${p.path}]: ${JSON.stringify(
                p.state,
              )}, context: ${JSON.stringify(p.context)}`,
            )
          },
          onSet: p => {
            messages.push(
              `onSet called, [${p.path}]: ${JSON.stringify(p.state)}, ${JSON.stringify(
                p.previous,
              )} => ${JSON.stringify(p.value)}, ${JSON.stringify(p.merged)}`,
            )
          },
          onDestroy: p => {
  ...
  expect(messages.slice(1)).toEqual([
    'onSet called, []: [{"f1":0,"f2":"str2"}], [{"f1":0,"f2":"str"}] => [{"f1":0,"f2":"str2"}], undefined',
  ])
  ...
  expect(messages.slice(2)).toEqual([
    'onSet called, [0,f1]: [{"f1":1,"f2":"str2"}], 0 => 1, undefined',
  ])
  ...
  expect(messages.slice(3)).toEqual([
    'onSet called, [0]: [{"f1":2,"f2":"str2"}], {"f1":2,"f2":"str2"} => {"f1":2,"f2":"str2"}, {"f1":2}',
  ])
  ...
  expect(messages.slice(5)).toEqual([
    'onBatchStart called, []: [{"f1":2,"f2":"str2"}], context: "custom context"',
    'batch executed, state: [{"f1":2,"f2":"str2"}]',
    'onBatchFinish called, []: [{"f1":2,"f2":"str2"}], context: "custom context"',
  ])
  ...
  expect(messages.slice(8)).toEqual([
    'onSet called, []: [{"f1":0,"f2":"str3"}], [{"f1":2,"f2":"str2"}] => [{"f1":0,"f2":"str3"}], undefined',
  ])
...
test('plugin: common flow callbacks global state', async () => {
  ...
  expect(renderTimes).toStrictEqual(1)
  expect(messages).toEqual(['onInit called, initial: [{"f1":0,"f2":"str"}]'])
  expect(result.current[0].get().f1).toStrictEqual(0)
  expect(messages).toEqual(['onInit called, initial: [{"f1":0,"f2":"str"}]'])
  ...
  expect(renderTimes).toStrictEqual(2)
  expect(messages.slice(1)).toEqual([
    'onSet called, [0,f1]: [{"f1":1,"f2":"str"}], 0 => 1, undefined',
  ])
  ...
  expect(renderTimes).toStrictEqual(3)
  expect(messages.slice(2)).toEqual([
    'onSet called, [0]: [{"f1":2,"f2":"str"}], {"f1":2,"f2":"str"} => {"f1":2,"f2":"str"}, {"f1":2}',
  ])
  ...
  expect(renderTimes).toStrictEqual(3)
  expect(messages.slice(4)).toEqual([
    'onSet called, [0,f1]: [{"f1":3,"f2":"str"}], 2 => 3, undefined',
  ])
...
test('plugin: common flow callbacks devtools', async () => {
    ...
    ({
      ...
      init: l => {
        ...
        return {
          ...
          onSet: p => {
            messages.push(
              `${label} onSet called, [${p.path}]: ${JSON.stringify(p.state)}, ${JSON.stringify(
                p.previous,
              )} => ${JSON.stringify(p.value)}, ${JSON.stringify(p.merged)}`,
            )
          },
  ...
  try {
    ...
    expect(renderTimes).toStrictEqual(2)
    expect(messages.slice(1)).toEqual([
      'LABELLED onSet called, [0,f1]: [{"f1":1,"f2":"str"}], 0 => 1, undefined',
    ])
    ...
    expect(renderTimes).toStrictEqual(3)
    expect(messages.slice(2)).toEqual([
      'LABELLED onSet called, [0]: [{"f1":2,"f2":"str"}], {"f1":2,"f2":"str"} => {"f1":2,"f2":"str"}, {"f1":2}',
    ])
...
test('plugin: common flow callbacks global state devtools', async () => {
  ...
  try {
    ...
    expect(renderTimes).toStrictEqual(1)
    expect(messages).toEqual(['undefined onInit called, initial: [{"f1":0,"f2":"str"}]'])
    expect(result.current[0].get().f1).toStrictEqual(0)
    expect(messages).toEqual(['undefined onInit called, initial: [{"f1":0,"f2":"str"}]'])
