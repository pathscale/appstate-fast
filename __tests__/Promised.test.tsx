import { useState, createState, none } from '../dist'
...
test('primitive: should rerender used on promise resolve', async () => {
  ...
  expect(() => result.current.keys).toThrow(
    'Error: HOOKSTATE-103 [path: /]. See https://hookstate.js.org/docs/exceptions#hookstate-103',
  )
  expect(() => result.current.get()).toThrow(
    'Error: HOOKSTATE-103 [path: /]. See https://hookstate.js.org/docs/exceptions#hookstate-103',
  )

  expect(() => result.current.set(200)).toThrow(
    'Error: HOOKSTATE-104 [path: /]. See https://hookstate.js.org/docs/exceptions#hookstate-104',
  )
...
test('primitive: should rerender used on promise resolve manual', async () => {
  ...
  expect(renderTimes).toStrictEqual(1)
  expect(result.current.promised).toStrictEqual(true)
  expect(() => result.current.value).toThrow(
    'Error: HOOKSTATE-103 [path: /]. See https://hookstate.js.org/docs/exceptions#hookstate-103',
  )
  expect(() => result.current.get()).toThrow(
    'Error: HOOKSTATE-103 [path: /]. See https://hookstate.js.org/docs/exceptions#hookstate-103',
